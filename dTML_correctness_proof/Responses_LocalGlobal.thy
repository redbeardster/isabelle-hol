(*Showing that all the response  events' annotations are locally correct and stable with respect to all the DTML operations*)
theory Responses_LocalGlobal
imports  DTML
begin



lemma Begin_response_Begin_invocation_global:
  assumes "thrdsvars"
and "  Begin_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Begin_invocation_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Begin_response tb    \<sigma> \<sigma>'"
and "total_wfs (\<tau> \<sigma>) "
  and "((pc \<sigma>) ta) \<in>  {Aborted,BeginPending,NotStarted} "
  and " ((pc \<sigma>)tb) \<in> {Aborted,BeginResponding,Ready}  " 
and " ta \<noteq> tb"
shows "Begin_invocation_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Begin_invocation_inv_def Begin_Response_inv_def  TML_Begin_response_def total_wfs_def  )
  by (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )


lemma Begin_response_Begin_response_global:
  assumes "thrdsvars"
and "  Begin_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
...
shows "Begin_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add: Begin_Response_inv_def TML_Begin_response_def total_wfs_def  )
...
lemma Begin_response_Read_invocation_global:
  assumes "thrdsvars"
and "  Begin_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
...
shows "Read_invocation_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Begin_Response_inv_def  TML_Begin_response_def  Read_invocation_inv_def  total_wfs_def  )
  apply  (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )
  apply (unfold Ready_total_def)
  by metis+
...
lemma Begin_response_Read_response_global:
  assumes "thrdsvars"
and "  Begin_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
...
shows "Read_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Begin_Response_inv_def  TML_Begin_response_def  Read_Response_inv_def  total_wfs_def  )
  apply   (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )
  apply (unfold Ready_total_def)
  by  metis+
...
lemma Begin_response_Write_invocation_global:
  assumes "thrdsvars"
and "  Begin_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
...
shows "Write_invocation_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Begin_Response_inv_def  TML_Begin_response_def  Write_invocation_inv_def  total_wfs_def  )
  apply   (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )
...
lemma Begin_response_Write_response_global:
  assumes "thrdsvars"
and "  Begin_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
...
shows "Write_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Begin_Response_inv_def  TML_Begin_response_def  Write_Response_inv_def  total_wfs_def  )
  apply   (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )
...
lemma Begin_response_Commit_invocation_global:
  assumes "thrdsvars"
and "  Begin_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
...
shows "Commit_invocation_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Begin_Response_inv_def  TML_Begin_response_def  Commit_invocation_inv_def  total_wfs_def  )
...
lemma Begin_response_Commit_response_global:
  assumes "thrdsvars"
...
shows "Begin_invocation_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Begin_invocation_inv_def  TML_Begin_response_def  Commit_Response_inv_def  total_wfs_def  )
...
lemma Read_response_Begin_invocation_global:
  assumes "thrdsvars"
and "  Read_invocation_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Begin_invocation_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Read_response  tb    \<sigma> \<sigma>'"
and "total_wfs (\<tau> \<sigma>) "
  and " ((pc \<sigma>)ta) \<in> {Aborted,BeginPending,NotStarted}  " 
  and "((pc \<sigma>) tb) \<in>  {Aborted,ReadResponding, Ready} "
and " ta \<noteq> tb"
shows "Begin_invocation_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Begin_invocation_inv_def Read_Response_inv_def   TML_Read_response_def total_wfs_def  )
...
lemma Read_response_Begin_response_global:
  assumes "thrdsvars"
and "  Read_invocation_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Begin_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Read_response  tb    \<sigma> \<sigma>'"
...
shows "Begin_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Begin_Response_inv_def Read_Response_inv_def   TML_Read_response_def total_wfs_def  )
...
lemma Read_response_Read_invocation_global:
  assumes "thrdsvars"
and "  Read_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Read_invocation_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Read_response  tb    \<sigma> \<sigma>'"
and "total_wfs (\<tau> \<sigma>) "
  and "((pc \<sigma>) ta) \<in>  {Aborted,ReadPending,Ready} "
...
shows "Read_invocation_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Read_invocation_inv_def  TML_Read_response_def  Read_Response_inv_def  total_wfs_def  )
...
lemma Read_response_Read_response_global:
  assumes "thrdsvars"
and "  Read_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Read_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Read_response tb    \<sigma> \<sigma>'"
...
shows "Read_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add: TML_Read_response_def  Read_Response_inv_def  total_wfs_def  )
...
lemma Read_response_Write_invocation_global:
  assumes "thrdsvars"
and "  Read_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Write_invocation_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Read_response  tb    \<sigma> \<sigma>'"
...
shows "Write_invocation_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Read_invocation_inv_def  TML_Read_response_def  _Write_invocation_inv_def  total_wfs_def  )
...
lemma Read_reasponse_Write_response_global:
  assumes "thrdsvars"
and "  Read_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Write_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Read_response  tb    \<sigma> \<sigma>'"
...
shows "Write_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Read_Response_inv_def  TML_Read_response_def  Write_Response_inv_def  total_wfs_def  )
...
lemma Read_response_Commit_invocation_global:
  assumes "thrdsvars"
and "  Read_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Commit_invocation_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Read_response  tb    \<sigma> \<sigma>'"
...
shows "Commit_invocation_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Read_Response_inv_def  TML_Read_response_def  Commit_invocation_inv_def  total_wfs_def  )
  apply   (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )
...
lemma Read_response_Commit_response_global:
  assumes "thrdsvars"
and "  Read_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Commit_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Read_response  tb    \<sigma> \<sigma>'"
...
lemma Write_response_Begin_invocation_global:
  assumes "thrdsvars"
and "  Write_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Begin_invocation_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Write_response tb    \<sigma> \<sigma>'"
and "total_wfs (\<tau> \<sigma>) "
  and " ((pc \<sigma>)ta) \<in> {Aborted,BeginPending,NotStarted}  " 
  and "((pc \<sigma>) tb) \<in>  {Aborted,WriteResponding, Ready} "
and " ta \<noteq> tb"
shows "Begin_invocation_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Begin_invocation_inv_def Write_Response_inv_def   TML_Write_response_def total_wfs_def  )

  by (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )



lemma Write_response_Begin_response_global:
  assumes "thrdsvars"
and "  Write_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Begin_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Write_response  tb    \<sigma> \<sigma>'"
...
shows "Begin_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Begin_Response_inv_def Write_Response_inv_def   TML_Write_response_def total_wfs_def  )
...
lemma  Write_response_Read_invocation_global:
  assumes "thrdsvars"
and "  Write_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Read_invocation_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Write_response  tb    \<sigma> \<sigma>'"
...
shows "Read_invocation_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Read_invocation_inv_def  TML_Write_response_def  Write_Response_inv_def  total_wfs_def  )
...
lemma  Write_response_Read_response_global:
  assumes "thrdsvars"
  and "Read_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Write_response  tb    \<sigma> \<sigma>'"
and "total_wfs (\<tau> \<sigma>) "
...
shows "Read_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Read_Response_inv_def  TML_Write_response_def  Write_Response_inv_def  total_wfs_def  )
...
lemma Write_response_Write_invocation_global:
  assumes "thrdsvars"
and "  Write_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Write_invocation_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Write_response  tb    \<sigma> \<sigma>'"
...
shows "Write_invocation_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:  TML_Write_response_def  _Write_invocation_inv_def _Write_Response_inv_def  total_wfs_def  )
...
lemma Write_response_Write_response_global:
  assumes "thrdsvars"
and "  Write_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Write_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Write_response  tb    \<sigma> \<sigma>'"
...
shows "Write_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:  TML_Write_response_def  _Write_Response_inv_def _Write_Response_inv_def  total_wfs_def  )
...
lemma Write_response_Commit_invocation_global:
  assumes "thrdsvars"
and "  Write_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Commit_invocation_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Write_response  tb    \<sigma> \<sigma>'"
...
shows "Commit_invocation_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Write_Response_inv_def  TML_Write_response_def  Commit_invocation_inv_def  total_wfs_def  )
...
lemma Write_response_Commit_response_global:
  assumes "thrdsvars"
and "  Write_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Commit_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
...
shows "Commit_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  by(simp add:Write_Response_inv_def  TML_Write_response_def  Commit_Response_inv_def  total_wfs_def  )
...
lemma Commit_response_Begin_invocation_global:
  assumes "thrdsvars"
and "  Commit_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Begin_invocation_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Commit_response  tb    \<sigma> \<sigma>'"
and "total_wfs (\<tau> \<sigma>) "
  and " ((pc \<sigma>)ta) \<in> {Aborted,BeginPending,NotStarted}  " 
  and "((pc \<sigma>) tb) \<in>  {Aborted,CommitResponding, Committed} "
and " ta \<noteq> tb"
shows "Begin_invocation_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Begin_invocation_inv_def Commit_invocation_inv_def   TML_Commit_response_def total_wfs_def  )
...
lemma Commit_response_Begin_response_global:
  assumes "thrdsvars"
and "  Commit_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Begin_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Commit_response  tb    \<sigma> \<sigma>'"
...
shows "Begin_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Begin_Response_inv_def   TML_Commit_response_def total_wfs_def  )
...
lemma Commit_response_Read_invocation_global:
  assumes "thrdsvars"
and "  Commit_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Read_invocation_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Commit_response  tb    \<sigma> \<sigma>'"
...
shows "Read_invocation_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Read_invocation_inv_def Commit_invocation_inv_def   TML_Commit_response_def total_wfs_def  )
...
lemma Commit_response_Read_response_global:
  assumes "thrdsvars"
and "  Commit_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Read_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Commit_response  tb    \<sigma> \<sigma>'"
...
shows "Read_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Read_Response_inv_def Commit_Response_inv_def   TML_Commit_response_def total_wfs_def  )
...
lemma Commit_response_Write_invocation_global:
  assumes "thrdsvars"
and "  Commit_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Write_invocation_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Commit_response  tb    \<sigma> \<sigma>'"
...
shows "Write_invocation_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Write_invocation_inv_def Commit_invocation_inv_def    TML_Commit_response_def total_wfs_def  )
...
lemma Commit_response_Write_response_global:
  assumes "thrdsvars"
and "  Commit_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Write_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Commit_response  tb    \<sigma> \<sigma>'"
...
shows "Write_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Write_Response_inv_def Commit_Response_inv_def   TML_Commit_response_def total_wfs_def  )
...
lemma Commit_response_Commit_invocation_global:
  assumes "thrdsvars"
and "  Commit_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Commit_invocation_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Commit_response  tb    \<sigma> \<sigma>'"
...
shows "Commit_invocation_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add: Commit_invocation_inv_def   TML_Commit_response_def  Commit_invocation_inv_def  total_wfs_def  )
...
lemma Commit_response_Commit_response_global:
  assumes "thrdsvars"
and "  Commit_Response_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Commit_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Commit_response  tb    \<sigma> \<sigma>'"
...
shows "Commit_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  by(simp add: Commit_Response_inv_def TML_Commit_response_def Commit_invocation_inv_def  total_wfs_def  )
...
shows "Begin_Response_inv t  ((pc \<sigma>') t) \<sigma>'"  
  using assms
  apply (simp add: TML_Begin_response_def Begin_Response_inv_def Ready_total_def  total_wfs_def   )
  apply (cases "pc \<sigma> t";simp  )
  apply (unfold Ready_total_def)
  by metis


lemma Begin_Begin_response_global:
  assumes "thrdsvars"
...
shows "Begin_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Begin_Response_inv_def Begin_inv_def TML_Begin_def total_wfs_def  )
  apply (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )
...
lemma Begin_response_Begin_global:
  assumes "thrdsvars"
and "  Begin_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Begin_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
...
shows "  Begin_inv  tb   ((pc \<sigma>') tb)  \<sigma>'" 
  using assms
  apply(simp add:Begin_Response_inv_def Begin_inv_def total_wfs_def TML_Begin_response_def Ready_total_def )
  apply  (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )
  apply (unfold Ready_total_def read_pre_def)
  by (smt (verit))
...
lemma Read_Begin_response_global:
  assumes "thrdsvars"
...
shows "Begin_Response_inv   ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Begin_Response_inv_def Read_inv_def TML_Read_def total_wfs_def  )
  apply (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )
  apply (unfold Ready_total_def read_pre_def)
...
apply (case_tac " \<not> readAux \<sigma> ta \<and>   \<not> writeAux \<sigma> ta ")
        apply simp
  apply (metis assms(5) lastVal_ni ld_ov_ni reg_same_ld_dr)
  apply simp
  apply (intro conjI)
  apply (metis reg_same_ld_dt)
  apply clarify
  apply (smt (verit) assms(5) ld_coh_dt_ni ld_coh_valof_dt_ni ld_le_lim_ni ld_vrnew_dt_ni reg_same_ld_dr)
  apply (metis reg_same_ld_dt)
...
apply (case_tac " \<not> readAux \<sigma> ta \<and>   \<not> writeAux \<sigma> ta ")
        apply simp
  apply (smt (verit) assms(5) cas_fail_diff_lv cas_lastVal cas_le_daddr cas_ov_daddr_dt_sx_ni cas_succ_v2_in_x insert_absorb insert_iff insert_not_empty numeral_1_eq_Suc_0 numerals(1) reg_same_CAS_dt total_wfs_def zero_neq_one)
  apply simp
  apply (intro conjI)
...
apply (case_tac " \<not> readAux \<sigma> ta \<and>   \<not> writeAux \<sigma> ta ")
 ...
 apply (metis cas_possible_lv_lc lastVal_def reg_same_CAS_dt)
  ...
  apply (metis assms(5) cas_ov_daddr_dt_sx_ni)
  apply blast
  apply (metis cas_possible_lv_lc lastVal_def reg_same_CAS_dt)
           apply simp
   apply (subgoal_tac "   even (regs (\<tau> \<sigma>') ta DTML.loc) \<and>
...
lemma Begin_response_Read_global:
  assumes "thrdsvars"
and "  Read_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Begin_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
 and " TML_Begin_response  ta   \<sigma> \<sigma>'"
...
shows "  Read_inv  tb   ((pc \<sigma>') tb)  \<sigma>'" 
  using assms
  apply(simp add:Begin_Response_inv_def Read_inv_def total_wfs_def TML_Begin_response_def Ready_total_def )
  apply  (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )
...
lemma Write_Begin_response_global:
  assumes "thrdsvars"
...
shows "Begin_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Begin_Response_inv_def Write_inv_def TML_Write_def total_wfs_def  )
  apply (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )
      apply (unfold Ready_total_def read_pre_def )
  apply (smt (verit) fun_upd_other)
  apply (smt (verit) fun_upd_other)
...
apply (case_tac " \<not> readAux \<sigma> ta \<and>   \<not> writeAux \<sigma> ta ")
  ...
  apply (metis reg_same_CAS_dt)
  apply simp
  apply (metis (no_types, lifting) assms(5) cas_coh_dt_ni cas_coh_valof_dt_ni cas_le_lim_dt_ni cas_vrnew_dt_ni reg_same_CAS_dt)
  apply (metis reg_same_CAS_dt)
                apply ( simp add: split: if_split_asm)+
...
apply (case_tac " \<not> readAux \<sigma> ta \<and>   \<not> writeAux \<sigma> ta ")
  apply simp
  apply (metis even_Suc reg_dt_ni reg_write_lastVal_ni)
  apply simp
  apply (intro conjI)
  apply (metis reg_dt_ni)
  apply clarify
...
apply (case_tac " \<not> readAux \<sigma> ta \<and>   \<not> writeAux \<sigma> ta ")
  ...
  apply (metis even_Suc reg_dt_ni reg_write_lastVal_ni)
             apply simp
  using  upreg_read_pre_ni read_pre_def
  apply (metis (no_types, opaque_lifting) assms(1) assms(5))
...
apply (case_tac " \<not> readAux \<sigma> ta \<and>   \<not> writeAux \<sigma> ta ")
  ...
  apply (metis reg_same_ld_dt)
  apply clarify
  apply (smt (verit) assms(5) ld_coh_dt_ni ld_coh_valof_dt_ni ld_le_lim_ni ld_vrnew_dt_ni reg_same_ld_dt)
  apply (metis reg_same_ld_dt)
...
apply (case_tac " \<not> readAux \<sigma> ta \<and>   \<not> writeAux \<sigma> ta ")
  ...
  apply (metis (no_types, lifting) fun_upd_other map_upd_eqD1)
...
apply (case_tac " \<not> readAux \<sigma> ta \<and>   \<not> writeAux \<sigma> ta ")
  ...
  apply (metis (no_types, lifting) assms(5) reg_coh_dt_ni reg_same_st st_coh_valof_dt_ni st_le_lim_dt_ni st_vrnew_dt_ni)
  apply (metis reg_same_st)
...
apply (case_tac " \<not> readAux \<sigma> ta \<and>   \<not> writeAux \<sigma> ta ")
  ...
  using read_pre_def flo_read_pre_ni
  apply (metis assms(5) thrdsvars_def)
  apply (metis reg_same_flo)
...
lemma Begin_Response_Write_global:
  assumes "thrdsvars"
and "  Write_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Begin_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
 and " TML_Begin_response  ta   \<sigma> \<sigma>'"
...
shows "  Write_inv  tb   ((pc \<sigma>') tb)  \<sigma>'" 
  using assms
  apply(simp add:Begin_Response_inv_def Write_inv_def total_wfs_def TML_Begin_response_def Ready_total_def )
  apply  (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )
  apply (unfold Ready_total_def read_pre_def)
  apply (smt (verit))
  apply metis
  apply (metis PC.distinct(355))
  apply (metis PC.distinct(355))
  apply (metis PC.distinct(355))
  apply metis
  apply metis
...
lemma Commit_Begin_response_global:
  assumes "thrdsvars"
...
shows "Begin_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Begin_Response_inv_def Commit_inv_def TML_Commit_def total_wfs_def  )
...
apply (case_tac " \<not> readAux \<sigma> ta \<and>   \<not> writeAux \<sigma> ta ")
              apply simp
        apply (metis PC.distinct(493) lastVal_def reg_same_sfence sf_nlv_ni)
  apply simp
...
lemma Begin_response_Commit_global:
  assumes "thrdsvars"
and "  Commit_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Begin_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
 and " TML_Begin_response  ta   \<sigma> \<sigma>'"
...
shows "  Commit_inv  tb   ((pc \<sigma>') tb)  \<sigma>'" 
  using assms
  apply(simp add:Begin_Response_inv_def Commit_inv_def total_wfs_def TML_Begin_response_def Ready_total_def )
...
lemma Read_response_local:
  assumes "thrdsvars"
and "total_wfs (\<tau> \<sigma>) "
and "Read_Response_inv t  ((pc \<sigma>) t) \<sigma>" 
 and " TML_Read_response  t   \<sigma> \<sigma>'"
shows " Read_Response_inv t  ((pc \<sigma>') t) \<sigma>'"  
  using assms
  apply (simp add: TML_Read_response_def Read_Response_inv_def Ready_total_def   total_wfs_def   )
  apply (cases "pc \<sigma> t";simp  )
  apply (simp add: Ready_total_def)
  by (metis PC.distinct(1109))
...
lemma Begin_Read_response_global:
  assumes "thrdsvars"
and "  Begin_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Read_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Begin  tb    \<sigma> \<sigma>'"
...
shows "Read_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
 ...
 apply (unfold thrdsvars_def )
  apply (simp add:TML_Begin_def Begin_inv_def Read_Response_inv_def total_wfs_def  )
...
lemma Read_response_Begin_global:
  assumes "thrdsvars"
and "  Begin_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Read_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
...
shows "  Begin_inv  tb   ((pc \<sigma>') tb)  \<sigma>'" 
  using assms
  apply(simp add:Read_Response_inv_def Begin_inv_def total_wfs_def TML_Read_response_def Ready_total_def )
  apply  (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )
  apply (metis PC.distinct(1109))
  apply (unfold Ready_total_def read_pre_def)
  apply (metis PC.distinct(1109))
  by (smt (verit))
...
lemma Read_Read_response_global:
  assumes "thrdsvars"
and "  Read_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Read_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Read  tb    \<sigma> \<sigma>'"
...
shows "Read_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
 apply (unfold thrdsvars_def )
  apply (simp add:TML_Read_def Read_inv_def Read_Response_inv_def  )
 apply (cases "(pc \<sigma>) ta";simp; cases "(pc \<sigma>) tb" ;simp)
...
lemma Read_response_Read_global:
  assumes "thrdsvars"
and "  Read_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Read_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
 and " TML_Read_response  ta   \<sigma> \<sigma>'"
...
shows "  Read_inv  tb   ((pc \<sigma>') tb)  \<sigma>'" 
  using assms
  apply(simp add:Read_Response_inv_def Read_inv_def total_wfs_def TML_Read_response_def Ready_total_def )
  apply  (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )
  apply (unfold Ready_total_def read_pre_def)
  apply (smt (verit))
  apply metis
...
lemma Write_Read_response_global:
  assumes "thrdsvars"
and "  Write_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Read_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Write  tb    \<sigma> \<sigma>'"
...
shows "Read_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply (unfold thrdsvars_def )
  apply (simp add:TML_Write_def Write_inv_def Read_Response_inv_def read_pre_def Ready_total_def  )
  apply   (cases "(pc \<sigma>) ta";simp; cases "(pc \<sigma>) tb" ;simp)
  apply (simp add: Ready_total_def)
...
apply (case_tac "   \<not> readAux \<sigma> ta \<and>
  ...
  apply (metis (no_types, lifting) PC.distinct(1109) PC.distinct(1273) assms(5) read_pre_def thrdsvars_def)
                  apply (simp add: split if_splits)
...
lemma Read_response_Write_global:
  assumes "thrdsvars"
and "  Write_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Read_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
 and " TML_Read_response  ta   \<sigma> \<sigma>'"
...
shows "  Write_inv  tb   ((pc \<sigma>') tb)  \<sigma>'" 
  using assms
  apply(simp add:Read_Response_inv_def Write_inv_def total_wfs_def TML_Read_response_def Ready_total_def read_pre_def )
  apply  (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )
  apply (unfold Ready_total_def read_pre_def )
  apply (smt (verit))
  apply metis
...
lemma Commit_Read_response_global:
  assumes "thrdsvars"
and "  Commit_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Read_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Commit  tb    \<sigma> \<sigma>'"
...
shows "Read_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
 apply (unfold thrdsvars_def )
  apply (simp add:TML_Commit_def Commit_inv_def Read_Response_inv_def total_wfs_def )
 ...
 apply (unfold Ready_total_def)
  ...
  apply (case_tac " \<not> readAux \<sigma> ta \<and>   \<not> writeAux \<sigma> ta ")
        apply simp
          apply simp
  apply (metis PC.distinct(1109) PC.distinct(493) assms(1) assms(5) reg_same_sfence sf_read_pre_ni)
...
lemma Read_response_Commit_global:
  assumes "thrdsvars"
and "  Commit_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Read_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
 and " TML_Read_response  ta   \<sigma> \<sigma>'"
...
shows "  Commit_inv  tb   ((pc \<sigma>') tb)  \<sigma>'" 
  using assms
  apply(simp add:Read_Response_inv_def Commit_inv_def total_wfs_def TML_Read_response_def Ready_total_def read_pre_def )
  apply (cases "(pc \<sigma>) ta";simp; cases "(pc \<sigma>) tb" ;simp)
  apply (unfold Ready_total_def)
  apply metis
  apply (metis PC.distinct(1109))
...
lemma Begin_Write_response_global:
  assumes "thrdsvars"
and "  Begin_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Write_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Begin  tb    \<sigma> \<sigma>'"
...
shows "Write_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
 apply (unfold thrdsvars_def )
  apply (simp add:TML_Begin_def Begin_inv_def Write_Response_inv_def  )
  apply  (cases "(pc \<sigma>) ta";simp; cases "(pc \<sigma>) tb" ;simp)
...
lemma Write_response_Begin_global:
  assumes "thrdsvars"
and "  Begin_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Write_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
...
shows "  Begin_inv  tb   ((pc \<sigma>') tb)  \<sigma>'" 
  using assms
  apply(simp add:Write_Response_inv_def Begin_inv_def total_wfs_def TML_Write_response_def  )
  apply  (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )
  apply (metis PC.distinct(1459))
  apply (metis PC.distinct(1459))
  apply (unfold Ready_total_def read_pre_def)
  by (smt (verit))
...
lemma Read_Write_response_global:
  assumes "thrdsvars"
and "  Read_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Write_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Read  tb    \<sigma> \<sigma>'"
...
shows "Write_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
 apply (unfold thrdsvars_def )
  apply (simp add:TML_Read_def Read_inv_def Write_Response_inv_def  )
apply  (cases "(pc \<sigma>) ta";simp; cases "(pc \<sigma>) tb" ;simp)
                apply (unfold total_wfs_def read_pre_def Ready_total_def)
...
apply (case_tac " \<not> readAux \<sigma> ta \<and>   \<not> writeAux \<sigma> ta ")
  ...
  apply (smt (verit) PC.distinct(859) assms(5) lastVal_ni ld_ov_ni reg_same_ld_dr)
                apply simp
  apply (metis PC.distinct(1459) PC.distinct(859) assms(1) assms(5) read_pre_def read_read_pre_ni)
...
apply (case_tac " \<not> readAux \<sigma> ta \<and>   \<not> writeAux \<sigma> ta ")
  ...
  apply (metis reg_same_CAS_dt)
              apply clarify
  apply (metis PC.distinct(1459) assms(1) assms(5) cas_read_pre_ni read_pre_def)
...
lemma Write_response_Read_global:
  assumes "thrdsvars"
and "  Read_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Write_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
 and " TML_Write_response  ta   \<sigma> \<sigma>'"
...
shows "  Read_inv  tb   ((pc \<sigma>') tb)  \<sigma>'" 
  using assms
  apply(simp add:Write_Response_inv_def Read_inv_def total_wfs_def TML_Write_response_def  )
  apply  (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )
...
lemma Write_Write_reponse_global:
  assumes "thrdsvars"
and "  Write_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Write_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Write  tb    \<sigma> \<sigma>'"
...
shows "Write_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Write_Response_inv_def Write_inv_def total_wfs_def TML_Write_def  )
 apply  (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )
  apply (unfold total_wfs_def read_pre_def Ready_total_def)
...
apply (case_tac " \<not> readAux \<sigma> ta \<and>   \<not> writeAux \<sigma> ta ")
 ...
 apply (case_tac "  odd (regs (\<tau> \<sigma>) ta DTML.loc)")
  ...
  apply clarify
  using  upreg_read_pre_ni
  apply (metis (no_types, lifting) PC.distinct(1459) assms(5) read_pre_def thrdsvars_def)
...
apply (case_tac "  odd (regs (\<tau> \<sigma>) ta DTML.loc)")
 ...
 apply simp
  ...
  apply (metis PC.distinct(1433))
    ...
    apply (intro conjI)
      apply (metis reg_same_flo)
  apply (metis PC.distinct(1459) assms(1) assms(5) flo_read_pre_ni read_pre_def)
  apply (metis reg_same_flo)
  apply presburger
...
lemma Write_response_Write_global:
  assumes "thrdsvars"
and "  Write_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Write_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
 and " TML_Write_response  ta   \<sigma> \<sigma>'"
...
shows "  Write_inv  tb   ((pc \<sigma>') tb)  \<sigma>'" 
  using assms
  apply(simp add:Write_Response_inv_def Write_inv_def total_wfs_def  TML_Write_response_def  )
  apply  (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )
  apply (unfold total_wfs_def read_pre_def Ready_total_def)
  apply (smt (verit))
  apply metis
...
lemma Commit_Write_response_global:
  assumes "thrdsvars"
and "  Commit_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Write_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Commit  tb    \<sigma> \<sigma>'"
...
shows "Write_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
  apply(simp add:Write_Response_inv_def Commit_inv_def total_wfs_def TML_Commit_def  )
apply  (cases" pc  \<sigma> ta"; simp ; cases "(pc \<sigma>) tb"; simp )
  ...
  apply (  cases" pc \<sigma>' ta ";simp)
           apply (simp add: Ready_total_def)
  apply (smt (verit) PC.distinct(425) fun_upd_other)
  apply (metis PC.distinct(1481) fun_upd_def)
         apply (simp add:  Ready_total_def)
  apply (smt(verit) PC.distinct(1459) PC.distinct(493) assms(1) assms(5) assms(8) reg_same_sfence sf_oav_ni sf_ots_ni sf_ov_ni sf_read_pre_ni sf_wfs_preserved singletonD step_mem_sfence total_wfs_def)
...
apply (case_tac "  odd (regs (\<tau> \<sigma>) ta DTML.loc)")
 ...
 apply (case_tac " \<not> readAux \<sigma> ta \<and>   \<not> writeAux \<sigma> ta ")
  ...
  apply (metis PC.distinct(1459) PC.distinct(623) assms(1) assms(5))
  apply (  cases" pc \<sigma>' ta ";simp)
         apply (simp add:  Ready_total_def)
  apply (metis PC.distinct(1481) pc_aux)
...
apply (case_tac "  odd (regs (\<tau> \<sigma>) ta DTML.loc)")
 ...
 apply (case_tac " \<not> readAux \<sigma> ta \<and>   \<not> writeAux \<sigma> ta ")
  ...
  apply (  cases" pc \<sigma>' ta ";simp)
   apply (simp add:  Ready_total_def)
  apply (metis PC.distinct(1483) pc_aux)
...
lemma Write_response_Commit_global:
  assumes "thrdsvars"
and "  Commit_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Write_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
 and " TML_Write_response  ta   \<sigma> \<sigma>'"
...
shows "  Commit_inv  tb   ((pc \<sigma>') tb)  \<sigma>'" 
  using assms
  apply(simp add:Write_Response_inv_def Commit_inv_def total_wfs_def  TML_Write_response_def  )
  apply  (cases" pc  \<sigma> ta"; simp ; cases "(pc \<sigma>) tb"; simp )
  apply (unfold Ready_total_def)
  apply metis
  apply (metis (no_types, opaque_lifting) PC.distinct(1459))
  apply (metis (no_types, opaque_lifting) PC.distinct(1459))
  apply (metis PC.distinct(1459))
  by metis+
...
lemma Begin_Commit_response_global:
  assumes "thrdsvars"
and "  Begin_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Commit_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Begin  tb    \<sigma> \<sigma>'"
...
shows "Commit_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
 apply (unfold thrdsvars_def )
  by (simp add:TML_Begin_def Begin_inv_def Commit_Response_inv_def  )


lemma Commit_response_Begin_global:
  assumes "thrdsvars"
and "  Begin_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Commit_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
...
shows "  Begin_inv  tb   ((pc \<sigma>') tb)  \<sigma>'" 
  using assms
  apply(simp add:Commit_Response_inv_def Begin_inv_def total_wfs_def TML_Commit_response_def  )
  apply  (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )
  apply (metis PC.distinct(685))
  apply (metis PC.distinct(685))
  apply (unfold Ready_total_def read_pre_def)
  by (smt (verit))


lemma Read_Commit_response_global:
  assumes "thrdsvars"
and "  Read_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Commit_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Read  tb    \<sigma> \<sigma>'"
...
shows "Commit_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
 apply (unfold thrdsvars_def )
  by (simp add:TML_Read_def Read_inv_def Commit_Response_inv_def  )



lemma Commit_response_Read_global:
  assumes "thrdsvars"
and "  Read_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Commit_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
 and " TML_Commit_response  ta   \<sigma> \<sigma>'"
...
shows "  Read_inv  tb   ((pc \<sigma>') tb)  \<sigma>'" 
  using assms
  apply(simp add:Commit_Response_inv_def Read_inv_def total_wfs_def TML_Commit_response_def  )
  apply  (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )
    apply (unfold total_wfs_def read_pre_def Ready_total_def)
  apply (smt (verit))
  apply metis
...
lemma Write_Commit_response_global:
  assumes "thrdsvars"
and "  Write_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Commit_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Write  tb    \<sigma> \<sigma>'"
...
shows "Commit_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
 apply (unfold thrdsvars_def )
  by (simp add:TML_Write_def Write_inv_def Commit_Response_inv_def  )
 

lemma Commit_response_Write_global:
  assumes "thrdsvars"
and "  Write_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Commit_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
 and " TML_Commit_response  ta   \<sigma> \<sigma>'"
...
shows "  Write_inv  tb   ((pc \<sigma>') tb)  \<sigma>'" 
  using assms
  apply(simp add:Commit_Response_inv_def Write_inv_def total_wfs_def TML_Commit_response_def  )
  apply  (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )
   apply (unfold total_wfs_def read_pre_def Ready_total_def)
  by metis+


lemma Commit_Commit_response_global:
  assumes "thrdsvars"
and "  Commit_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Commit_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
   and " TML_Commit  tb    \<sigma> \<sigma>'"
...
shows "Commit_Response_inv ta  ((pc \<sigma>') ta) \<sigma>'" 
  using assms
 apply (unfold thrdsvars_def )
by (simp add:TML_Commit_def Commit_inv_def Commit_Response_inv_def  )
  


lemma Commit_response_Commit_global:
  assumes "thrdsvars"
and "  Commit_inv  tb   ((pc \<sigma>) tb)  \<sigma>  "
and "Commit_Response_inv ta  ((pc \<sigma>) ta) \<sigma>" 
 and " TML_Commit_response  ta   \<sigma> \<sigma>'"
...
shows "  Commit_inv  tb   ((pc \<sigma>') tb)  \<sigma>'" 
  using assms
  apply(simp add:Commit_Response_inv_def Commit_inv_def total_wfs_def TML_Commit_response_def  )
  apply  (cases" pc  \<sigma> tb"; simp ; cases "(pc \<sigma>) ta"; simp )
   apply (unfold total_wfs_def read_pre_def Ready_total_def)
  by metis+
...
shows "Write_Response_inv t  ((pc \<sigma>') t) \<sigma>'"  
  using assms
  apply (simp add: TML_Write_response_def Write_Response_inv_def   total_wfs_def   )
  apply (cases "pc \<sigma> t";simp  )
  apply (unfold Ready_total_def read_pre_def)
  by force


lemma Commit_response_local:
  assumes "thrdsvars"
and "total_wfs (\<tau> \<sigma>) "
and "Commit_Response_inv t  ((pc \<sigma>) t) \<sigma>" 
 and " TML_Commit_response  t   \<sigma> \<sigma>'"
shows "Commit_Response_inv t  ((pc \<sigma>') t) \<sigma>'"  
  using assms
  by(simp add: TML_Commit_response_def Commit_Response_inv_def   total_wfs_def   )
