...
next
  ...
  proof
    ...
    also have "\<dots> < 2*fact n" using IH[OF A] by simp 
    also have "\<dots> < (n+1) * fact n" using PREM by simp
    also have "\<dots> = fact (Suc n)" by simp
    finally show ?thesis .    
  next
    assume "n=3" then show ?thesis by (simp add: eval_nat_numeral)
  qed      
  (*  
  have ?case proof cases
    assume "3<n" then show ?thesis sorry
  next
    assume "\<not>(3<n)" 
...
lemma sum_of_naturals: "2 * sumto (\<lambda>x. x) n = n * Suc n" by (induction n) auto


lemma "sumto (\<lambda>x. x) n ^ 2 = sumto (\<lambda>x. x^3) n"
proof (induction n)
  case 0 show ?case by simp (* by simp Should be trivial.  *)
next
  case (Suc n)
  ...
  have "(sumto (\<lambda>x. x) (Suc n))\<^sup>2 = ((Suc n) + sumto (\<lambda>x. x) n)\<^sup>2"  
    by simp
  also have "\<dots> = (Suc n)\<^sup>2 + 2*sumto (\<lambda>x. x) n*(Suc n) + (sumto (\<lambda>x. x) n)\<^sup>2"
    by (simp add: eval_nat_numeral)
  also have "\<dots> = (Suc n)\<^sup>2 + (Suc n)*n*(Suc n) + (sumto (\<lambda>x. x) n)\<^sup>2"    
    by (simp add: sum_of_naturals)
  also have "\<dots> = (Suc n)^3 + (sumto (\<lambda>x. x) n)\<^sup>2"    
    by (simp add: eval_nat_numeral)
  also have "\<dots> = (Suc n)^3 + sumto (\<lambda>x. x ^ 3) n" by (simp add: IH)
...
fun path :: "('a \<Rightarrow> 'a \<Rightarrow> bool) \<Rightarrow> 'a \<Rightarrow> 'a list \<Rightarrow> 'a \<Rightarrow> bool" 
  ...
  | "path G u (w#p) v \<longleftrightarrow> G u w \<and> path G w p v" 
...
proof -
  ...
  from not_distinct_decomp[OF ND] obtain xs ys zs y 
    where [simp]: "p=xs @ [y] @ ys @ [y] @ zs" by blast
      
  let ?p'="(xs@[y]@ys@[y]@ys@[y]@zs)"    
  from P have "path G u ?p' v" by auto
  then have "length ?p' > length p \<and> \<not>distinct ?p' \<and> path G u ?p' v" by simp
