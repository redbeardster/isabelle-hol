...
where
"contains _ [] = False" |
"contains x (y#r) = (if x=y then True else (contains x r))"
...
(* Un lemme intermédiaire pour la preuve du dessous, sa preuve n'est pas intéressante, 
    on peut la sauter *)
lemma temp: "((contains e l) \<and> (nth i l = e)) \<longrightarrow> (\<exists>i. nth i (a#l) = e)"
...
lemma "(contains e l) \<longrightarrow> (\<exists> i. (nth i l) = e)" 
apply (induct l)
...
(* Il ne reste plus qu'un sous-but pour le cas a\<noteq>e.*)
                              (* Dans ce cas, les hypothèses (contains e (a#l)) et (e\<noteq>a) font
                                 que l'on obtient l'hypothèse (contains e l) en appliquant 
                                 la définition de contains avec simp. *)
apply simp                   
                              (* Avec (contains e l), on doit pouvoir utiliser l'hypothèse d'induction
                                 (contains e l \<longrightarrow> (\<exists>i. pc.nth i l = e)) et obtenir (\<exists>i. pc.nth i l = e)
                              *)
apply (case_tac "\<not>(\<exists>i. pc.nth i l = e)")
apply simp  by (meson temp)
...
lemma "((contains (Pay tid 0) l)) \<longrightarrow> \<not>(g l)"
...
apply (metis pc.contains.simps(1))
...
(* En utilisant slegehammer comme une commande de preuve, si nécessaire on peut:
   ...
   - supprimer des lemmes à considérer (s'ils peuvent l'emmener dans une mauvaise direction) avec 
     (del: ...)
 *)
sledgehammer [provers=e cvc4 spass z3 remote_vampire,timeout=120] (del: pc.contains.simps(2) add:temp)
  by (metis list.sel(3) contains.elims(2) pc.nth.simps(1) temp)
