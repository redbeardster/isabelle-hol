...
section \<open>Priority Manager Formal Verification with Strong Typing\<close>

subsection \<open>Сильно типизированные базовые типы\<close>
...
proof -
  show "\<exists>x. x \<in> {n::nat. n \<ge> 10 \<and> n \<le> 95}"
...
proof -
  show "\<exists>x. x \<in> {n::nat. n \<le> 100}"
    by (rule exI[of _ 0]) simp
qed

typedef memory_type = "{n::nat. n \<le> 16384}"
  morphisms memory_val mk_memory
proof -
  show "\<exists>x. x \<in> {n::nat. n \<le> 16384}"
    by (rule exI[of _ 0]) simp
qed
...
lemma memory_bounds [simp]:
  "memory_val m \<le> 16384"
  using memory_val by auto

subsection \<open>Безопасные операции над типами\<close>

lift_definition decrease_priority :: "priority_type \<Rightarrow> priority_type" is
  "\<lambda>x. if x > 10 then max 10 (x - 10) else 10"
  by auto

lift_definition decrease_priority_small :: "priority_type \<Rightarrow> priority_type" is
  "\<lambda>x. if x > 10 then max 10 (x - 5) else 10"  
  by auto

lift_definition decrease_priority_minimal :: "priority_type \<Rightarrow> priority_type" is
  "\<lambda>x. if x > 10 then max 10 (x - 3) else 10"
  by auto
...
subsection \<open>Состояние системы\<close>

record system_state =
  processes :: "process_id set"
  priorities :: "process_id \<Rightarrow> priority_type" 
...
definition init_state :: system_state where
  "init_state = \<lparr>
    processes = {},
    priorities = (\<lambda>_. MIN_PRIORITY),
...
subsection \<open>Упрощённые инварианты\<close>

text \<open>Инвариант 1: Приоритеты в границах (теперь гарантирован типом!)\<close>
definition priorities_in_bounds :: "system_state \<Rightarrow> bool" where
  "priorities_in_bounds s \<equiv> True"

text \<open>Инвариант 2: Количество процессов ограничено\<close>
definition process_count_bounded :: "nat \<Rightarrow> system_state \<Rightarrow> bool" where
  "process_count_bounded max_procs s \<equiv> 
    finite (processes s) \<and> card (processes s) \<le> max_procs"

text \<open>Инвариант 3: Нагрузка в пределах (гарантирован типом!)\<close>  
definition load_bounded :: "system_state \<Rightarrow> bool" where
  "load_bounded s \<equiv> True"

text \<open>Инвариант 4: Память в пределах (гарантирован типом!)\<close>
definition memory_valid :: "system_state \<Rightarrow> bool" where
  "memory_valid s \<equiv> True"

text \<open>Общий инвариант\<close>
definition type_ok :: "nat \<Rightarrow> system_state \<Rightarrow> bool" where
  "type_ok max_procs s \<equiv>
    process_count_bounded max_procs s"

subsection \<open>Действия системы\<close>

definition discover_process :: "process_id \<Rightarrow> priority_type \<Rightarrow> system_state \<Rightarrow> system_state" where
  "discover_process pid prio s = s\<lparr>
    processes := processes s \<union> {pid},
    priorities := (priorities s)(pid := prio)
  \<rparr>"

definition terminate_process :: "process_id \<Rightarrow> system_state \<Rightarrow> system_state" where  
  "terminate_process pid s = s\<lparr>
    processes := processes s - {pid}
  \<rparr>"

definition adjust_priority_by_load :: "process_id \<Rightarrow> system_state \<Rightarrow> system_state" where
  "adjust_priority_by_load pid s = 
    (if pid \<in> processes s \<and> load_val (system_load s) > 50
     then s\<lparr>
...
definition adjust_priority_by_responsiveness :: "process_id \<Rightarrow> system_state \<Rightarrow> system_state" where
  "adjust_priority_by_responsiveness pid s = 
    (if pid \<in> processes s \<and> responsiveness s > 2000
     then s\<lparr>
...
definition adjust_priority_by_memory :: "process_id \<Rightarrow> system_state \<Rightarrow> system_state" where
  "adjust_priority_by_memory pid s = 
    (if pid \<in> processes s \<and> memory_val (available_memory s) < 100
     then s\<lparr>
...
text \<open>Начальное состояние корректно\<close>
lemma init_state_type_ok:
  "type_ok max_procs init_state"
  unfolding type_ok_def process_count_bounded_def init_state_def
  by auto

text \<open>Все действия сохраняют границы приоритетов (теперь тривиально!)\<close>
...
text \<open>Главная теорема сохранения инвариантов\<close>
theorem priority_bounds_invariant:
  "priorities_in_bounds (discover_process pid prio s) \<and>
   priorities_in_bounds (terminate_process pid s) \<and>
   priorities_in_bounds (adjust_priority_by_load pid s) \<and>
   priorities_in_bounds (adjust_priority_by_responsiveness pid s) \<and>
   priorities_in_bounds (adjust_priority_by_memory pid s)"
  by (simp add: priorities_in_bounds_def)

subsection \<open>Свойства значений (value-level proofs)\<close>

text \<open>Приоритет уменьшается при высокой нагрузке\<close>
theorem priority_decreases_under_load:
  assumes "pid \<in> processes s"
...
text \<open>Строгое уменьшение когда возможно\<close>
...
text \<open>Счётчик корректировок увеличивается\<close>
...
text \<open>Прогресс к минимальному приоритету\<close>
lemma progress_towards_min_priority:
  assumes "pid \<in> processes s"
      and "load_val (system_load s) > 50"
      and "priority_val (priorities s pid) > 10"
  shows "priority_val (priorities (adjust_priority_by_load pid s) pid) \<le>
         priority_val (priorities s pid)"
proof -
  have "priority_val (priorities (adjust_priority_by_load pid s) pid) = 
...
subsection \<open>Точные свойства корректировки приоритетов\<close>

text \<open>Точное уменьшение когда возможно\<close>
...
text \<open>Гарантированное не-увеличение\<close>
...
text \<open>Достижение минимума\<close>
lemma reaches_minimum:
  assumes "pid \<in> processes s"
      and "load_val (system_load s) > 50" 
      and "priority_val (priorities s pid) \<le> 20"
...
text \<open>Строгое уменьшение при достаточном запасе\<close>
...
text \<open>Монотонность по отношению к исходному приоритету\<close>
lemma adjustment_monotonic:
  assumes "pid1 \<in> processes s" "pid2 \<in> processes s"
      and "load_val (system_load s) > 50"
      and "priority_val (priorities s pid1) \<le> priority_val (priorities s pid2)"
  shows "priority_val (priorities (adjust_priority_by_load pid1 s) pid1) \<le>
...
proof -
  ...
  also have "priority_val (priorities (adjust_priority_by_load pid2 s) pid2) = 
    ...
    using assms(4)   by (simp add: \<open>priority_val (priorities (adjust_priority_by_load pid2 s) pid2) = max 10 (priority_val (priorities s pid2) - 10)\<close> calculation diff_le_mono)
qed

(**)


text \<open>Конечное достижение минимума\<close>
theorem eventually_reaches_min_priority:
  assumes "pid \<in> processes s"
      and "load_val (system_load s) > 50" 
      and "priority_val (priorities s pid) > 10"
  shows "\<exists>n. n \<le> (priority_val (priorities s pid) - 10) div 10 + 1 \<and>
         priority_val (priorities ((adjust_priority_by_load pid ^^ n) s) pid) = 10"
  oops 


subsection \<open>Композиционные свойства\<close>

text \<open>Независимость корректировок разных процессов\<close>
theorem adjustment_independence:
  assumes "pid1 \<noteq> pid2"
      and "pid1 \<in> processes s" 
      and "pid2 \<in> processes s"
...
text \<open>Коммутативность для разных процессов\<close>  
 text \<open>Коммутативность для разных процессов\<close>  
...
subsection \<open>Метатеоремы\<close>

text \<open>Все действия сохраняют тип системы\<close>
theorem all_actions_preserve_type:
  assumes "type_ok max_procs s"
      and "card (processes s) < max_procs" 
    shows "type_ok max_procs (discover_process pid prio s) \<and>
           type_ok max_procs (terminate_process pid s) \<and>  
           type_ok max_procs (adjust_priority_by_load pid s) \<and>
           type_ok max_procs (adjust_priority_by_responsiveness pid s) \<and>
